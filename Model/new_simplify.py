...
i=0
address=""
address_ary = input_model.split("/");
#print (address_ary[0])

while i< len(address_ary)-1:
  x="/"
  y=str(address_ary[i])
  address= address+ y + x
  i+=1
...
bpy.ops.export_scene.obj(filepath="input/" + output_model)
...
allfilelist= os.listdir(str(address))
...
for file in allfilelist:
    if (path.isfile("input/"+file)== True):
      break
    else:
      print("starting copy")
      shutil.copy(str(address) +file, "input/")
