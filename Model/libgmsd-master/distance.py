...
with open(infile, "r") as inp, open("Objects/finalObj/model.txt", "w") as out,  open("I_output.txt", "w") as output:
            ...
            if o.type == 'MESH':
              ...
              # print( "max z is "+str(Max_z )+" and actual z-dim is :"+ str(bpy.data.objects['Camera'].location.z))
               ...
               if distance<Maxdistance :
                 ...
                 final_adr= output_dir+objname + "deg"+ str(angle)+"d"+ str(round(distance,2))+ "r"+str((new_ratio))+ ".bmp"

                 bpy.context.scene.render.filepath = output_dir+objname + "deg"+ str(angle)+"d"+ str(round(distance,2))+ "r"+str((new_ratio))
                 bpy.ops.render.render(write_still = True)
                 out.write(output_dir+objname + "deg"+ str(angle)+"d"+ str(round(distance,2))+ "r"+str((new_ratio))+ ".bmp"+"\n")
                ...
                # print( "changed dim x, y, z is "+ str(bpy.data.objects['Camera'].location.x)+ str(bpy.data.objects['Camera'].location.y)+ str(bpy.data.objects['Camera'].location.z ))
                 '''
               #  print ("first line is "+ first_adr + "\n")
                 #print ("last line is "+ final_adr + "\n")
                 final ="wine gmsd.exe %s %s > Final.txt" %(first_adr, final_adr)
                 os.system(final)
                 with open("Final.txt", "r") as iqa:
                   entry=iqa.readline()
                   output.write(entry)
                   print ("iqa measurement is "+entry)
              ...
              # print("before decimation object {} has {} verts, {} edges, {} polys".format(o.name, len(o.data.vertices), len(o.data.edges), len(o.data.polygons)))
               ...
               if old_ratio> 0.05:
                 new_ratio=old_ratio- float(0.05) # 0.8 as new ratio
               else:
                 new_ratio=old_ratio- float(0.005)
